...
const TalkStep2: React.FC<TalkStep2Props> = ({ control, setValue, errors, onBack, isSubmitting }) => {
    const relocationStage = useWatch({ control, name: 'relocationStage' }) || ''
    const selectedPlaces: string[] = useWatch({ control, name: 'selectedPlaces' }) || []
    const scoutingPeople = useWatch({ control, name: 'scoutingPeople' }) || ''

    const togglePlace = (place: string) => {
        if (place === "I'm not sure yet") {
            setValue('selectedPlaces', ["I'm not sure yet"], { shouldValidate: true, shouldDirty: true })
            return
        }

        const current = selectedPlaces.filter((p) => p !== "I'm not sure yet")
        let updated: string[]
        if (current.includes(place)) {
            updated = current.filter((p) => p !== place)
        } else {
            updated = [...current, place]
        }
        setValue('selectedPlaces', updated, { shouldValidate: true, shouldDirty: true })
    }
    ...
    return (
        <div className="space-y-8">
            ...
            {/* Question 1: Stage */}
            <div>
                <h4 className="text-base sm:text-lg font-medium text-title mb-1">
                    ...
                    {stageOptions.map((option) => {
                        ...
                        return (
                            <button
                                key={option}
                                type="button"
                                onClick={() => setValue('relocationStage', option, { shouldValidate: true, shouldDirty: true })}
                                className={`w-full text-left px-4 py-3 rounded-2xl border text-sm font-normal transition-all flex items-center justify-between cursor-pointer ${
                                    isSelected
                                        ? 'border-primary bg-red-50/20 text-title ring-1 ring-primary font-medium'
                    ...
                    {peopleOptions.map((people) => {
                        ...
                        return (
                            <button
                                key={people}
                                type="button"
                                onClick={() => setValue('scoutingPeople', people, { shouldValidate: true, shouldDirty: true })}
